...
exports.criarForma_pagamento = async (req, res) => {
  ...
  try {
    const { id_forma_pagamento, nome_forma} = req.body;

    // Validação básica
    if (!nome_forma) {
      return res.status(400).json({
        error: 'Nome do forma_pagamento é obrigatório'
      });
    }

    const result = await query(
      'INSERT INTO forma_pagamento (id_forma_pagamento, nome_forma) VALUES ($1, $2) RETURNING *',
      [id_forma_pagamento, nome_forma]
    );
...
exports.atualizarForma_pagamento = async (req, res) => {
  try {
    ...
    const updatedFields = {
      nome_forma: nome_forma !== undefined ? nome_forma : currentPerson.nome_forma
    };

    // Atualiza a forma_pagamento
    const updateResult = await query(
      'UPDATE forma_pagamento SET nome_forma = $1 WHERE id_forma_pagamento = $2 RETURNING *',
      [updatedFields.nome_forma, id]
    );
